...
/*    public static void updateEntry(Story s, Story old) {
    ...
    public static LinkedList<Course> getAll(Coach c) throws SQLException {
        LinkedList<Course> res = new LinkedList<>();
        try {
            con = getCon();
            String query = "select * from courses "
                    + "where cid = '%s'";

            query = String.format(query, c.getUid().toString());
            st = con.prepareStatement(query);
            ResultSet rs = st.executeQuery();
            while (rs.next()) {
                Course co = new Course(UUID.fromString(rs.getString("coid")),
                            rs.getString("name"),
                            rs.getInt("kuota"),
                            c, rs.getString("materi"),
                            rs.getString("soal"));
                res.add(co);
            }
        } catch (SQLException e) {
    ...
    public static LinkedList<Course> getAllforuser() throws SQLException {
        LinkedList<Course> res = new LinkedList<>();
        try {
            con = getCon();
            String query = "select * from courses ";
